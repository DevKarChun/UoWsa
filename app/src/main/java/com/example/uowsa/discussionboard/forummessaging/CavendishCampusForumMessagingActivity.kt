...
import com.example.uowsa.adapter.forumadapter.CavendishCampusMessageAdapter
import com.example.uowsa.usermodel.forummessagingmodel.CavendishCampusMessage
...
class CavendishCampusForumMessagingActivity : AppCompatActivity() {

    private var accommodationMessageList = ArrayList<CavendishCampusMessage>()
    private var scrollDown: ScrollView? = null
    private lateinit var auth: FirebaseAuth
    private var cavendishCampusForumMessagingRecyclerView: RecyclerView? = null
    private var cavendishCampusCreatorId: String? = null
    private var cavendishCampusCreatedDateTime : String? = null

    @SuppressLint("CutPasteId")
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_cavendish_campus_forum_messaging)

        auth = Firebase.auth

        scrollDown = findViewById(R.id.scrollDown)
        cavendishCampusForumMessagingRecyclerView = findViewById(R.id.cavendishCampusForumMessagingRecyclerView)
        ...
        cavendishCampusCreatorId = intent.getStringExtra("creatorId").toString()
        ...
        cavendishCampusCreatedDateTime = intent.getStringExtra("createdDateTime").toString()
        ...
        val databaseRef = FirebaseDatabase.getInstance().getReference("Users").child(cavendishCampusCreatorId!!)
        ...
        cavendishCampusForumMessagingRecyclerView?.layoutManager = layoutManager
        cavendishCampusForumMessagingRecyclerView?.layoutManager = LinearLayoutManager(this, RecyclerView.VERTICAL, false)
    ...
    private fun sendMessage(message: String) {
        ...
        val sendMessageDatabase = FirebaseDatabase.getInstance().getReference("CavendishCampusMessage").child(cavendishCampusCreatorId!!).child(cavendishCampusCreatedDateTime!!)
        ...
        messagingContent["senderCavendishCampusId"] = userId
        messagingContent["cavendishCampusMessage"] = message
        messagingContent["cavendishCampusMessageDateTime"] = dateTime
        messagingContent["cavendishCampusCreatorId"] = cavendishCampusCreatorId!!
        messagingContent["cavendishCampusCreatedDateTime"] = cavendishCampusCreatedDateTime!!
    ...
    private fun deleteAllMessages() {
        ...
        val userRef = FirebaseDatabase.getInstance().getReference("CavendishCampusMessage").child(cavendishCampusCreatorId!!).child(cavendishCampusCreatedDateTime!!)

        val userMessageDelete = userRef.orderByChild("senderCavendishCampusId").equalTo(userId)
    ...
    private fun readMessage() {
        ...
        val readMessageDatabase = FirebaseDatabase.getInstance().getReference("CavendishCampusMessage").child(cavendishCampusCreatorId!!).child(cavendishCampusCreatedDateTime!!)
        Log.d("tagDatabase", readMessageDatabase.toString())

        readMessageDatabase.addValueEventListener(object : ValueEventListener {
            override fun onDataChange(snapshot: DataSnapshot) {
                accommodationMessageList.clear()
                for (dataSnapshot: DataSnapshot in snapshot.children){
                    val chat = dataSnapshot.getValue(CavendishCampusMessage::class.java)
                    Log.d("tagChat", chat.toString())
                    Log.d("tagUserId", userId)
                    Log.d("tagsenderId", chat!!.senderCavendishCampusId)
                    Log.d("tagfriendId", chat.cavendishCampusMessage)
                    Log.d("tagmessagetime", chat.cavendishCampusCreatedDateTime)

                    accommodationMessageList.add(chat)
                    scrollDown!!.scrollTo(0, scrollDown!!.bottom)
                }
                val cavendishCampusMessageAdapter = CavendishCampusMessageAdapter(this@CavendishCampusForumMessagingActivity, accommodationMessageList)

                cavendishCampusForumMessagingRecyclerView?.adapter = cavendishCampusMessageAdapter
                cavendishCampusForumMessagingRecyclerView?.scrollToPosition(accommodationMessageList.size-1)
            }
