...
import com.example.uowsa.adapter.forumadapter.OtherMessageAdapter
import com.example.uowsa.usermodel.forummessagingmodel.OtherMessage
...
class OtherForumMessagingActivity : AppCompatActivity() {

    private var otherList = ArrayList<OtherMessage>()
    private var scrollDown: ScrollView? = null
    private lateinit var auth: FirebaseAuth
    private var otherForumMessagingRecyclerView: RecyclerView? = null
    private var otherCreatorId: String? = null
    private var otherCreatedDateTime : String? = null

    @SuppressLint("CutPasteId")
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_other_forum_messaging)

        auth = Firebase.auth

        scrollDown = findViewById(R.id.scrollDown)
        otherForumMessagingRecyclerView = findViewById(R.id.otherForumMessagingRecyclerView)
        ...
        otherCreatorId = intent.getStringExtra("creatorId").toString()
        ...
        otherCreatedDateTime = intent.getStringExtra("createdDateTime").toString()
        ...
        val databaseRef = FirebaseDatabase.getInstance().getReference("Users").child(otherCreatorId!!)
        ...
        otherForumMessagingRecyclerView?.layoutManager = layoutManager
        otherForumMessagingRecyclerView?.layoutManager = LinearLayoutManager(this, RecyclerView.VERTICAL, false)
    ...
    private fun sendMessage(message: String) {
        ...
        val sendMessageDatabase = FirebaseDatabase.getInstance().getReference("OtherMessage").child(otherCreatorId!!).child(otherCreatedDateTime!!)
        ...
        messagingContent["senderOtherId"] = userId
        messagingContent["otherMessage"] = message
        messagingContent["otherMessageDateTime"] = dateTime
        messagingContent["otherCreatorId"] = otherCreatorId!!
        messagingContent["otherCreatedDateTime"] = otherCreatedDateTime!!
    ...
    private fun deleteAllMessages() {
        ...
        val userRef = FirebaseDatabase.getInstance().getReference("OtherMessage").child(otherCreatorId!!).child(otherCreatedDateTime!!)

        val userMessageDelete = userRef.orderByChild("senderOtherId").equalTo(userId)
    ...
    private fun readMessage() {
        ...
        val readMessageDatabase = FirebaseDatabase.getInstance().getReference("OtherMessage").child(otherCreatorId!!).child(otherCreatedDateTime!!)
        ...
        readMessageDatabase.addValueEventListener(object : ValueEventListener {
            override fun onDataChange(snapshot: DataSnapshot) {
                ...
                for (dataSnapshot: DataSnapshot in snapshot.children){
                    val chat = dataSnapshot.getValue(OtherMessage::class.java)
                    Log.d("tagChat", chat.toString())
                    Log.d("tagUserId", userId)
                    Log.d("tagsenderId", chat!!.senderOtherId)
                    Log.d("tagfriendId", chat.otherMessage)
                    Log.d("tagmessagetime", chat.otherMessageDateTime)

                    otherList.add(chat)
                    scrollDown!!.scrollTo(0, scrollDown!!.bottom)
                }
                val otherMessageAdapter = OtherMessageAdapter(this@OtherForumMessagingActivity, otherList)

                otherForumMessagingRecyclerView?.adapter = otherMessageAdapter
                otherForumMessagingRecyclerView?.scrollToPosition(otherList.size-1)
            }
