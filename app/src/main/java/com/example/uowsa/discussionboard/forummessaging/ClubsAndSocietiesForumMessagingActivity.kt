...
import com.example.uowsa.adapter.forumadapter.ClubsAndSocietiesMessageAdapter
...
class ClubsAndSocietiesForumMessagingActivity : AppCompatActivity() {

    private var clubsAndSocietiesMessageList = ArrayList<ClubsAndSocietiesMessage>()
    private var scrollDown: ScrollView? = null
    private lateinit var auth: FirebaseAuth
    private var clubsAndSocietiesForumMessagingRecyclerView: RecyclerView? = null
    private var clubsAndSocietiesCreatorId: String? = null
    private var clubsAndSocietiesCreatedDateTime : String? = null

    @SuppressLint("CutPasteId")
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_clubs_and_societies_forum_messaging)

        auth = Firebase.auth

        scrollDown = findViewById(R.id.scrollDown)
        clubsAndSocietiesForumMessagingRecyclerView = findViewById(R.id.clubsAndSocietiesForumMessagingRecyclerView)
        ...
        clubsAndSocietiesCreatorId = intent.getStringExtra("creatorId").toString()
        ...
        clubsAndSocietiesCreatedDateTime = intent.getStringExtra("createdDateTime").toString()
        ...
        val databaseRef = FirebaseDatabase.getInstance().getReference("Users").child(clubsAndSocietiesCreatorId!!)
        ...
        clubsAndSocietiesForumMessagingRecyclerView?.layoutManager = layoutManager
        clubsAndSocietiesForumMessagingRecyclerView?.layoutManager = LinearLayoutManager(this, RecyclerView.VERTICAL, false)
    ...
    private fun sendMessage(message: String) {
        ...
        val sendMessageDatabase = FirebaseDatabase.getInstance().getReference("ClubsAndSocietiesMessage").child(clubsAndSocietiesCreatorId!!).child(clubsAndSocietiesCreatedDateTime!!)
        ...
        messagingContent["senderClubsAndSocietiesId"] = userId
        messagingContent["clubsAndSocietiesMessage"] = message
        messagingContent["clubsAndSocietiesMessageDateTime"] = dateTime
        messagingContent["clubsAndSocietiesCreatorId"] = clubsAndSocietiesCreatorId!!
        messagingContent["clubsAndSocietiesCreatedDateTime"] = clubsAndSocietiesCreatedDateTime!!
    ...
    private fun deleteAllMessages() {
        ...
        val userRef = FirebaseDatabase.getInstance().getReference("ClubsAndSocietiesMessage").child(clubsAndSocietiesCreatorId!!).child(clubsAndSocietiesCreatedDateTime!!)

        val userMessageDelete = userRef.orderByChild("senderClubsAndSocietiesId").equalTo(userId)
    ...
    private fun readMessage() {
        ...
        val readMessageDatabase = FirebaseDatabase.getInstance().getReference("ClubsAndSocietiesMessage").child(clubsAndSocietiesCreatorId!!).child(clubsAndSocietiesCreatedDateTime!!)
        Log.d("tagDatabase", readMessageDatabase.toString())

        readMessageDatabase.addValueEventListener(object : ValueEventListener {
            override fun onDataChange(snapshot: DataSnapshot) {
                clubsAndSocietiesMessageList.clear()
                for (dataSnapshot: DataSnapshot in snapshot.children){
                    val chat = dataSnapshot.getValue(ClubsAndSocietiesMessage::class.java)
                    Log.d("tagChat", chat.toString())
                    Log.d("tagUserId", userId)
                    Log.d("tagsenderId", chat!!.senderClubsAndSocietiesId)
                    Log.d("tagfriendId", chat.clubsAndSocietiesCreatorId)
                    Log.d("tagmessagetime", chat.clubsAndSocietiesCreatedDateTime)

                    clubsAndSocietiesMessageList.add(chat)
                    scrollDown!!.scrollTo(0, scrollDown!!.bottom)
                }
                val clubsAndSocietiesMessageAdapter = ClubsAndSocietiesMessageAdapter(this@ClubsAndSocietiesForumMessagingActivity, clubsAndSocietiesMessageList)

                clubsAndSocietiesForumMessagingRecyclerView?.adapter = clubsAndSocietiesMessageAdapter
                clubsAndSocietiesForumMessagingRecyclerView?.scrollToPosition(clubsAndSocietiesMessageList.size-1)
            }
