...
import com.example.uowsa.adapter.forumadapter.StudentUnionMessageAdapter
import com.example.uowsa.usermodel.forummessagingmodel.StudentUnionMessage
...
class StudentUnionForumMessagingActivity : AppCompatActivity() {

    private var studentUnionMessageList = ArrayList<StudentUnionMessage>()
    private var scrollDown: ScrollView? = null
    private lateinit var auth: FirebaseAuth
    private var studentUnionForumMessagingRecyclerView: RecyclerView? = null
    private var studentUnionCreatorId: String? = null
    private var studentUnionCreatedDateTime : String? = null

    @SuppressLint("CutPasteId")
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_student_union_forum_messaging)

        auth = Firebase.auth

        scrollDown = findViewById(R.id.scrollDown)
        studentUnionForumMessagingRecyclerView = findViewById(R.id.studentUnionForumMessagingRecyclerView)
        ...
        studentUnionCreatorId = intent.getStringExtra("creatorId").toString()
        ...
        studentUnionCreatedDateTime = intent.getStringExtra("createdDateTime").toString()
        ...
        val databaseRef = FirebaseDatabase.getInstance().getReference("Users").child(studentUnionCreatorId!!)
        ...
        studentUnionForumMessagingRecyclerView?.layoutManager = layoutManager
        studentUnionForumMessagingRecyclerView?.layoutManager = LinearLayoutManager(this, RecyclerView.VERTICAL, false)
    ...
    private fun sendMessage(message: String) {
        ...
        val sendMessageDatabase = FirebaseDatabase.getInstance().getReference("StudentUnionMessage").child(studentUnionCreatorId!!).child(studentUnionCreatedDateTime!!)
        ...
        messagingContent["senderStudentUnionId"] = userId
        messagingContent["studentUnionMessage"] = message
        messagingContent["studentUnionMessageDateTime"] = dateTime
        messagingContent["studentUnionCreatorId"] = studentUnionCreatorId!!
        messagingContent["studentUnionCreatedDateTime"] = studentUnionCreatedDateTime!!
    ...
    private fun deleteAllMessages() {
        ...
        val userRef = FirebaseDatabase.getInstance().getReference("StudentUnionMessage").child(studentUnionCreatorId!!).child(studentUnionCreatedDateTime!!)

        val userMessageDelete = userRef.orderByChild("senderStudentUnionId").equalTo(userId)
    ...
    private fun readMessage() {
        ...
        val readMessageDatabase = FirebaseDatabase.getInstance().getReference("StudentUnionMessage").child(studentUnionCreatorId!!).child(studentUnionCreatedDateTime!!)
        Log.d("tagDatabase", readMessageDatabase.toString())

        readMessageDatabase.addValueEventListener(object : ValueEventListener {
            override fun onDataChange(snapshot: DataSnapshot) {
                studentUnionMessageList.clear()
                for (dataSnapshot: DataSnapshot in snapshot.children){
                    val chat = dataSnapshot.getValue(StudentUnionMessage::class.java)
                    Log.d("tagChat", chat.toString())
                    Log.d("tagUserId", userId)
                    Log.d("tagsenderId", chat!!.senderStudentUnionId)
                    Log.d("tagfriendId", chat.studentUnionMessage)
                    Log.d("tagmessagetime", chat.studentUnionCreatedDateTime)

                    studentUnionMessageList.add(chat)
                    scrollDown!!.scrollTo(0, scrollDown!!.bottom)
                }
                val studentUnionMessageAdapter = StudentUnionMessageAdapter(this@StudentUnionForumMessagingActivity, studentUnionMessageList)

                studentUnionForumMessagingRecyclerView?.adapter = studentUnionMessageAdapter
                studentUnionForumMessagingRecyclerView?.scrollToPosition(studentUnionMessageList.size-1)
            }
