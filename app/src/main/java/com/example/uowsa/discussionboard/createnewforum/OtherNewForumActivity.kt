...
class OtherNewForumActivity : AppCompatActivity() {

    private lateinit var auth: FirebaseAuth

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_other_new_forum)
        ...
        submitBtn.setOnClickListener {
            if (titleTxt.text.toString().isEmpty()){
                ...
                createOtherForum(title, description)
                val intentStudy = Intent(this@OtherNewForumActivity, ForumOtherActivity::class.java)
    ...
    private fun createOtherForum(title: String, description: String) {
        Log.d("tag", "testing")
        ...
        val databaseRef = FirebaseDatabase.getInstance().getReference("OtherForum")
        Log.d("tagdatabaseref", databaseRef.toString())

        val newPost : HashMap <String, String> = HashMap()
        newPost["otherCreatorId"] = userId
        newPost["otherTitle"] = title
        newPost["otherDescription"] = description
        newPost["otherDate"] = date
        newPost["otherCreatedDateTime"] = dateTime
