...
import com.example.uowsa.adapter.forumadapter.EventsMessageAdapter
...
class EventsForumMessagingActivity : AppCompatActivity() {

    private var eventsMessageList = ArrayList<EventsMessage>()
    private var scrollDown: ScrollView? = null
    private lateinit var auth: FirebaseAuth
    private var eventsForumMessagingRecyclerView: RecyclerView? = null
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.activity_events_forum_messaging)

        auth = Firebase.auth

        scrollDown = findViewById(R.id.scrollDown)
        eventsForumMessagingRecyclerView = findViewById(R.id.eventsForumMessagingRecyclerView)
        ...
        val studyTitle = intent.getStringExtra("title").toString()
        val studyDescription = intent.getStringExtra("description").toString()
        val studyDate = intent.getStringExtra("date").toString()
        ...
        eventsForumMessagingRecyclerView?.layoutManager = layoutManager
        eventsForumMessagingRecyclerView?.layoutManager = LinearLayoutManager(this, RecyclerView.VERTICAL, false)
    ...
    private fun sendMessage(message: String) {
        ...
        val sendMessageDatabase = FirebaseDatabase.getInstance().getReference("EventsMessage").child(creatorId!!).child(createdDateTime!!)
        ...
        messagingContent["senderEventsId"] = userId
        messagingContent["eventsMessage"] = message
        messagingContent["eventsMessageDateTime"] = dateTime
        messagingContent["eventsCreatorId"] = creatorId!!
        messagingContent["eventsCreatedDateTime"] = createdDateTime!!

        Log.d("tag1", userId)
        Log.d("tag2", message)
        Log.d("tag3", dateTime)
        Log.d("tag4", creatorId!!)
        Log.d("tag5", createdDateTime!!)
    ...
    private fun deleteAllMessages() {
        ...
        val userRef = FirebaseDatabase.getInstance().getReference("EventsMessage").child(creatorId!!).child(createdDateTime!!)

        val userMessageDelete = userRef.orderByChild("senderEventsId").equalTo(userId)
    ...
    private fun readMessage() {
        ...
        val readMessageDatabase = FirebaseDatabase.getInstance().getReference("EventsMessage").child(creatorId!!).child(createdDateTime!!)
        Log.d("tagDatabase", readMessageDatabase.toString())

        readMessageDatabase.addValueEventListener(object : ValueEventListener {
            override fun onDataChange(snapshot: DataSnapshot) {
                eventsMessageList.clear()
                for (dataSnapshot: DataSnapshot in snapshot.children){
                    val chat = dataSnapshot.getValue(EventsMessage::class.java)
                    Log.d("tagChat", chat.toString())
                    Log.d("tagUserId", userId)
                    Log.d("tagsenderId", chat!!.eventsCreatorId)
                    Log.d("tagfriendId", chat.eventsMessage)
                    Log.d("tagmessagetime", chat.eventsMessageDateTime)

                    eventsMessageList.add(chat)
                    scrollDown!!.scrollTo(0, scrollDown!!.bottom)
                }
                val eventsMessageAdapter = EventsMessageAdapter(this@EventsForumMessagingActivity, eventsMessageList)

                eventsForumMessagingRecyclerView?.adapter = eventsMessageAdapter
                eventsForumMessagingRecyclerView?.scrollToPosition(eventsMessageList.size-1)
            }
