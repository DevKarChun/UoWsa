...
import com.example.uowsa.adapter.forumadapter.AccommodationMessageAdapter
import com.example.uowsa.usermodel.forummessagingmodel.AccommodationMessage
...
class AccommodationForumMessagingActivity : AppCompatActivity() {

    private var accommodationMessageList = ArrayList<AccommodationMessage>()
    private var scrollDown: ScrollView? = null
    private lateinit var auth: FirebaseAuth
    private var accommodationForumMessagingRecyclerView: RecyclerView? = null
    private var accommodationCreatorId: String? = null
    private var accommodationCreatedDateTime : String? = null


    @SuppressLint("CutPasteId")
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_accommodation_forum_messaging)

        auth = Firebase.auth

        scrollDown = findViewById(R.id.scrollDown)
        accommodationForumMessagingRecyclerView = findViewById(R.id.accommodationForumMessagingRecyclerView)
        ...
        accommodationCreatorId = intent.getStringExtra("creatorId").toString()
        ...
        accommodationCreatedDateTime = intent.getStringExtra("createdDateTime").toString()
        ...
        val databaseRef = FirebaseDatabase.getInstance().getReference("Users").child(accommodationCreatorId!!)
        ...
        accommodationForumMessagingRecyclerView?.layoutManager = layoutManager
        accommodationForumMessagingRecyclerView?.layoutManager = LinearLayoutManager(this, RecyclerView.VERTICAL, false)
    ...
    private fun sendMessage(message: String) {
        ...
        val sendMessageDatabase = FirebaseDatabase.getInstance().getReference("AccommodationMessage").child(accommodationCreatorId!!).child(accommodationCreatedDateTime!!)
        ...
        messagingContent["senderAccommodationId"] = userId
        messagingContent["accommodationMessage"] = message
        messagingContent["accommodationMessageDateTime"] = dateTime
        messagingContent["accommodationCreatorId"] = accommodationCreatorId!!
        messagingContent["accommodationCreatedDateTime"] = accommodationCreatedDateTime!!
    ...
    private fun deleteAllMessages() {
        ...
        val userRef = FirebaseDatabase.getInstance().getReference("AccommodationMessage").child(accommodationCreatorId!!).child(accommodationCreatedDateTime!!)

        val userMessageDelete = userRef.orderByChild("senderAccommodationId").equalTo(userId)
    ...
    private fun readMessage() {
        ...
        val readMessageDatabase = FirebaseDatabase.getInstance().getReference("AccommodationMessage").child(accommodationCreatorId!!).child(accommodationCreatedDateTime!!)
        ...
        readMessageDatabase.addValueEventListener(object : ValueEventListener {
            override fun onDataChange(snapshot: DataSnapshot) {
                ...
                for (dataSnapshot: DataSnapshot in snapshot.children){
                    val chat = dataSnapshot.getValue(AccommodationMessage::class.java)
                    Log.d("tagChat", chat.toString())
                    Log.d("tagUserId", userId)
                    Log.d("tagsenderId", chat!!.senderAccommodationId)
                    Log.d("tagfriendId", chat.accommodationMessage)
                    Log.d("tagmessagetime", chat.accommodationMessageDateTime)

                    accommodationMessageList.add(chat)
                    scrollDown!!.scrollTo(0, scrollDown!!.bottom)
                }
                val accommodationMessageAdapter = AccommodationMessageAdapter(this@AccommodationForumMessagingActivity, accommodationMessageList)

                accommodationForumMessagingRecyclerView?.adapter = accommodationMessageAdapter
                accommodationForumMessagingRecyclerView?.scrollToPosition(accommodationMessageList.size-1)
            }
