...
import com.example.uowsa.adapter.forumadapter.MaryleboneCampusMessageAdapter
...
class MaryleboneCampusForumMessagingActivity : AppCompatActivity() {

    private var maryleboneCampusMessageList = ArrayList<MaryleboneCampusMessage>()
    private var scrollDown: ScrollView? = null
    private lateinit var auth: FirebaseAuth
    private var maryleboneCampusForumMessagingRecyclerView: RecyclerView? = null
    private var maryleboneCampusCreatorId: String? = null
    private var maryleboneCampusCreatedDateTime : String? = null

    @SuppressLint("CutPasteId")
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_marylebone_campus_forum_messaging)

        auth = Firebase.auth

        scrollDown = findViewById(R.id.scrollDown)
        maryleboneCampusForumMessagingRecyclerView = findViewById(R.id.maryleboneCampusForumMessagingRecyclerView)
        ...
        maryleboneCampusCreatorId = intent.getStringExtra("creatorId").toString()
        ...
        maryleboneCampusCreatedDateTime = intent.getStringExtra("createdDateTime").toString()
        ...
        val databaseRef = FirebaseDatabase.getInstance().getReference("Users").child(maryleboneCampusCreatorId!!)
        ...
        maryleboneCampusForumMessagingRecyclerView?.layoutManager = layoutManager
        maryleboneCampusForumMessagingRecyclerView?.layoutManager = LinearLayoutManager(this, RecyclerView.VERTICAL, false)
    ...
    private fun sendMessage(message: String) {
        ...
        val sendMessageDatabase = FirebaseDatabase.getInstance().getReference("MaryleboneCampusMessage").child(maryleboneCampusCreatorId!!).child(maryleboneCampusCreatedDateTime!!)
        ...
        messagingContent["senderMaryleboneCampusId"] = userId
        messagingContent["maryleboneCampusMessage"] = message
        messagingContent["maryleboneCampusMessageDateTime"] = dateTime
        messagingContent["maryleboneCampusCreatorId"] = maryleboneCampusCreatorId!!
        messagingContent["maryleboneCampusCreatedDateTime"] = maryleboneCampusCreatedDateTime!!
    ...
    private fun deleteAllMessages() {
        ...
        val userRef = FirebaseDatabase.getInstance().getReference("MaryleboneCampusMessage").child(maryleboneCampusCreatorId!!).child(maryleboneCampusCreatedDateTime!!)

        val userMessageDelete = userRef.orderByChild("senderMaryleboneCampusId").equalTo(userId)
    ...
    private fun readMessage() {
        ...
        val readMessageDatabase = FirebaseDatabase.getInstance().getReference("MaryleboneCampusMessage").child(maryleboneCampusCreatorId!!).child(maryleboneCampusCreatedDateTime!!)
        Log.d("tagDatabase", readMessageDatabase.toString())

        readMessageDatabase.addValueEventListener(object : ValueEventListener {
            override fun onDataChange(snapshot: DataSnapshot) {
                maryleboneCampusMessageList.clear()
                for (dataSnapshot: DataSnapshot in snapshot.children){
                    val chat = dataSnapshot.getValue(MaryleboneCampusMessage::class.java)
                    Log.d("tagChat", chat.toString())
                    Log.d("tagUserId", userId)
                    Log.d("tagsenderId", chat!!.senderMaryleboneCampusId)
                    Log.d("tagfriendId", chat.maryleboneCampusMessage)
                    Log.d("tagmessagetime", chat.maryleboneCampusCreatedDateTime)

                    maryleboneCampusMessageList.add(chat)
                    scrollDown!!.scrollTo(0, scrollDown!!.bottom)
                }
                val maryleboneCampusMessageAdapter = MaryleboneCampusMessageAdapter(this@MaryleboneCampusForumMessagingActivity, maryleboneCampusMessageList)

                maryleboneCampusForumMessagingRecyclerView?.adapter = maryleboneCampusMessageAdapter
                maryleboneCampusForumMessagingRecyclerView?.scrollToPosition(maryleboneCampusMessageList.size-1)
            }
