...
import com.example.uowsa.discussionboard.dbprofile.DBMakeNewFriendsProfileActivity
...
class DBMakeNewFriendsAdapter (private val context: Context, private val dbMakeNewFriendsList: ArrayList<DBMakeNewFriends>)
    : RecyclerView.Adapter<DBMakeNewFriendsAdapter.ViewHolder>(){
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int){
        val dbMakeNewFriends = dbMakeNewFriendsList[position]
        holder.txtMessage.text = dbMakeNewFriends.DBMakeNewFriendsMessage
        val database = FirebaseDatabase.getInstance().getReference("Users").child(dbMakeNewFriends.senderDBMakeNewFriendsId)
        ...
        if (dbMakeNewFriends.senderDBMakeNewFriendsId == firebaseUser!!.uid){
        ...
        if (dbMakeNewFriends.senderDBMakeNewFriendsId != firebaseUser!!.uid){
            holder.circleImageLayout.setOnClickListener {
                val intent = Intent(context, DBMakeNewFriendsProfileActivity::class.java)
                intent.putExtra("friendUserId",dbMakeNewFriends.senderDBMakeNewFriendsId)
    ...
    private fun deleteMessage(position: Int) {
        ...
        val msgDateTime = dbMakeNewFriendsList[position].DBMakeNewFriendsMessageDateTime

        val databaseDateTimeMsgUser = FirebaseDatabase.getInstance().getReference("DBMakeNewFriends")

        if (dbMakeNewFriendsList[position].senderDBMakeNewFriendsId == userId){
            val timeUser = databaseDateTimeMsgUser.orderByChild("DBMakeNewFriendsMessageDateTime").equalTo(msgDateTime)
            timeUser.addListenerForSingleValueEvent(object: ValueEventListener {
                override fun onDataChange(snapshot: DataSnapshot) {
                    for (dataSnapShot12: DataSnapshot in snapshot.children){
                        if (dataSnapShot12.child("senderDBMakeNewFriendsId").getValue() == userId){
    ...
    override fun getItemCount(): Int {
        return dbMakeNewFriendsList.size
    }
    ...
    override fun getItemViewType(position: Int): Int {
        ...
        return if (dbMakeNewFriendsList[position].senderDBMakeNewFriendsId == firebaseUser!!.uid) {
            messageRight
        } else {
            messageLeft
        }
