...
import com.example.uowsa.discussionboard.forumprofile.StudyProfileActivity
...
class StudyMessageAdapter (private val context: Context, private val studyMessageList: ArrayList<StudyMessage>) : RecyclerView.Adapter<StudyMessageAdapter.ViewHolder>(){
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        val studyMessage = studyMessageList[position]
        holder.txtMessage.text = studyMessage.studyMessage
        val database = FirebaseDatabase.getInstance().getReference("Users").child(studyMessage.senderStudyId)
        ...
        if (studyMessage.senderStudyId == firebaseUser!!.uid){
        ...
        if (studyMessage.senderStudyId != firebaseUser!!.uid){
            holder.circleImageLayout.setOnClickListener {
                val intent = Intent(context, StudyProfileActivity::class.java)
                intent.putExtra("friendUserId",studyMessage.senderStudyId)
    ...
    private fun deleteMessage(position: Int) {
        ...
        val msgDateTime = studyMessageList[position].studyMessageDateTime

        val databaseDateTimeMsgUser = FirebaseDatabase.getInstance().getReference("StudyMessage").child(studyMessageList[position].studyCreatorId).child(studyMessageList[position].studyCreatedDateTime)

        if (studyMessageList[position].senderStudyId == userId){
            val timeUser = databaseDateTimeMsgUser.orderByChild("studyMessageDateTime").equalTo(msgDateTime)
            timeUser.addListenerForSingleValueEvent(object: ValueEventListener {
                override fun onDataChange(snapshot: DataSnapshot) {
                    for (dataSnapShot12: DataSnapshot in snapshot.children){
                        if (dataSnapShot12.child("senderStudyId").getValue() == userId){
    ...
    override fun getItemViewType(position: Int): Int {
        ...
        return if (studyMessageList[position].senderStudyId == firebaseUser!!.uid) {
            messageRight
        } else {
            messageLeft
        }
