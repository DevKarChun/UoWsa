...
import com.example.uowsa.discussionboard.forumprofile.OtherProfileActivity
...
class OtherMessageAdapter (private val context: Context, private val otherMessageList: ArrayList<OtherMessage>) : RecyclerView.Adapter<OtherMessageAdapter.ViewHolder>(){
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        val otherMessage = otherMessageList[position]
        holder.txtMessage.text = otherMessage.otherMessage
        val database = FirebaseDatabase.getInstance().getReference("Users").child(otherMessage.senderOtherId)
        ...
        if (otherMessage.senderOtherId == firebaseUser!!.uid){
        ...
        if (otherMessage.senderOtherId != firebaseUser!!.uid){
            holder.circleImageLayout.setOnClickListener {
                ...
                intent.putExtra("friendUserId",otherMessage.senderOtherId)
    ...
    private fun deleteMessage(position: Int) {
        ...
        val msgDateTime = otherMessageList[position].otherMessageDateTime

        val databaseDateTimeMsgUser = FirebaseDatabase.getInstance().getReference("OtherMessage").child(otherMessageList[position].otherCreatorId).child(otherMessageList[position].otherCreatedDateTime)

        if (otherMessageList[position].senderOtherId == userId){
            ...
            timeUser.addListenerForSingleValueEvent(object: ValueEventListener {
                override fun onDataChange(snapshot: DataSnapshot) {
                    for (dataSnapShot12: DataSnapshot in snapshot.children){
                        if (dataSnapShot12.child("senderOtherId").getValue() == userId){
    ...
    override fun getItemViewType(position: Int): Int {
        ...
        return if (otherMessageList[position].senderOtherId == firebaseUser!!.uid) {
            messageRight
        } else {
            messageLeft
        }
