...
import com.example.uowsa.discussionboard.forummessaging.StudyForumMessagingActivity
...
class StudyForumAdapter (private val context: Context, private val StudyForumList: ArrayList <StudyForum>) : RecyclerView.Adapter<StudyForumAdapter.ViewHolder>(){
    ...
    override fun getItemCount(): Int {
        return StudyForumList.size
    }

    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        val studyList = StudyForumList[position]
        holder.titleTxt.text = studyList.studyTitle
        holder.datePlaceholder.text = "Date: "
        holder.date.text = studyList.studyDate

        holder.userPlaceholder.text = "Posted by: "
        val id = studyList.creatorId
        ...
        holder.forumLayout.setOnClickListener {
            val intent = Intent(context, StudyForumMessagingActivity::class.java)
            intent.putExtra("creatorId", studyList.creatorId)
            intent.putExtra("studyTitle", studyList.studyTitle)
            intent.putExtra("studyDescription", studyList.studyDescription)
            intent.putExtra("studyDate", studyList.studyDate)
            intent.putExtra("createdDateTime", studyList.createdDateTime)
    ...
    private fun deleteMessage(position: Int) {
        ...
        val msgDateTime = StudyForumList[position].createdDateTime

        val databaseDateTimeMsgUser = FirebaseDatabase.getInstance().getReference("StudyForum")

        if (StudyForumList[position].creatorId == userId){
            val timeUser = databaseDateTimeMsgUser.orderByChild("createdDateTime").equalTo(msgDateTime)
            timeUser.addListenerForSingleValueEvent(object: ValueEventListener {
                override fun onDataChange(snapshot: DataSnapshot) {
                    for (dataSnapShot12: DataSnapshot in snapshot.children){
                        if (dataSnapShot12.child("creatorId").getValue() == userId){
        ...
        val userRef = FirebaseDatabase.getInstance().getReference("StudyForum").child(StudyForumList[position].creatorId).child(StudyForumList[position].createdDateTime)
        ...
        if (StudyForumList[position].creatorId == userId){
            val databaseUserForumRemoveContents = FirebaseDatabase.getInstance().getReference("AccommodationMessage").child(StudyForumList[position].creatorId).child(StudyForumList[position].createdDateTime)
    ...
    override fun getItemViewType(position: Int): Int {
        ...
        return if (StudyForumList[position].creatorId == firebaseUser!!.uid) {
            postCreator
        } else {
            post
        }
